...
- QR images and HLS browser code are served locally; setup does not contact a
  QR service or CDN.
  ...
  <div class="app-shell">
    ...
    <div class="screen" id="screen-4">
        ...
        <p class="helper-text">
            ...
            image is generated locally by NomWatch.)
        </p>
    ...
    <script>
        ...
        function renderQr() {{
            ...
            document.getElementById('qr-container').innerHTML =
                `<img src="/api/v1/qr?url=${{encodeURIComponent(url)}}" alt="QR code for ${{url}}">`;
        }}
...
def create_app(*, state=None, auth=None, supervisor=None, network_manager=None, gateway_http_get=None):
    ...
    def api_test_camera():
        ...
        from .network import NetworkPolicyError, validate_camera_target
        selected_networks = []
        if network_manager and network_manager.confirmed:
            selected_networks.append(network_manager.confirmed.network)
        try:
            pinned_ip = validate_camera_target(str(data["ip"]).strip(), port, selected_networks)
        except NetworkPolicyError as exc:
            return jsonify({"ok": False, "error": str(exc), "hints": []}), 422
    ...
    def api_check_mediamtx():
        ...
        return jsonify({
            "reachable": reachable,
            "url": "/api/v1/live/camera/index.m3u8",
            "mediamtx_installed": binary_available("mediamtx"),
        })
